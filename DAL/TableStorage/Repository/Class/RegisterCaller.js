...
function getAllForPlacefun(parameters){
    return new Promise(function(resolve, reject){
            ...
            tableService.queryEntities('RegisterCaller',Query, null, function(error, result, response){
                if(!error){
...
function insertFun(parameters){
    return new Promise(function(resolve, reject){
        tableService.createTableIfNotExists('RegisterCaller', function(error,result, response){
            if(!error){
                tableServiceButtons.insertEntity('RegisterCaller', parameters, function(error, result, response){
                    if(error){
                        return resolve(result.entries);
                    }else{
                        return reject(error);
...
function deleteFun(parameters){
    return new Promise(function(resolve, reject){
            ...
            tableService.deleteEntity('RegisterCaller',Query, null, function(error,result,response){
                if(!error){
                   return resolve(result.entries);
                }else{
                   return reject(error);
...
function updateOneByIdForPlacefun(parameters){
    return new Promise(function(resolve, reject){
        tableService.replaceEntity('RegisterCaller',parameters, null, function(error,result,response){
                if(!error){
                   return resolve(result.entries);
                }else{
                   return reject(error);
